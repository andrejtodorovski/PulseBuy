...
export class CartItemService {
    ...
    async create(createCartItemDto: CreateCartItemDto): Promise<CartItem> {
        ...
        if (createCartItemDto.quantity <= 0) {
            throw new NotAcceptableException('Quantity should be greater than 0');
        }
        if (!cart) {
        ...
        const productAlreadyInTheCart = await this.cartItemRepository.findOne({
            where: {cart: {id: createCartItemDto.cartId}, product: {id: createCartItemDto.productId}},
        });
        if (productAlreadyInTheCart) {
            throw new NotAcceptableException(`The product is already in the cart. Please update the quantity from the cart instead.`);
        }
