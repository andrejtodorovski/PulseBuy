<script lang="ts">
    import { onMount } from 'svelte';
    import MessagesRepository from "../repository/messagesRepository";
    import { toasts } from "svelte-toasts";
    import { SendMessageDto } from "../models/message";
    import { getSessionCookieValue, getUserId, isUserAdmin } from "../helpers/helpers";
    import { io } from "$lib/webSocketConnection";

    let showChat = false;
    let content = '';
    let messages = []

    function toggleChat() {
        showChat = !showChat;
        if (showChat) {
            loadMessages()
        }
    }

    async function loadMessages() {
        const userId = getUserId()
        let res;
        if (userId != null) {
            res = await MessagesRepository.getMessagesByUserId(userId)
        } else {
            const cookie = await getSessionCookieValue()
            res = await MessagesRepository.getMessagesByCookie(cookie)
        }
        const response = await res.json()

        if (res.ok) {
            messages = response
        } else {
            toasts.error(response.message)
        }
    }

    async function sendMessage() {
        if (content === '') {
            return
        }
        const userId = getUserId()
        let sendMessageDto;
        if (userId == null) {
            const cookie = await getSessionCookieValue()
            sendMessageDto = new SendMessageDto(
                content,
                cookie,
                null
            )
        } else {
            sendMessageDto = new SendMessageDto(
                content,
                null,
                userId
            )
        }
        await MessagesRepository.sendMessage(sendMessageDto)
        await loadMessages()
    }

    onMount(() => {
        const roomId = "Message";

        io.emit("joinRoom", roomId);

        io.on("Message.MessageSentByAdminEvent", (event) => {
            loadMessages()
        });
        document.addEventListener('click', handleClickOutside);
        return () => {
            document.removeEventListener('click', handleClickOutside);
        };
    });

    function handleKeyDown(event) {
        if (event.key === 'Enter') {
            sendMessage();
        }
    }

    function handleClickOutside(event) {
        const chatButton = document.querySelector('.chat-button');
        const chatInterface = document.querySelector('.chat-interface');

        if (chatButton && chatInterface && !chatButton.contains(event.target) && !chatInterface.contains(event.target)) {
            showChat = false;
        }
    }
</script>

<style>
    .chat-button {
        position: fixed;
        bottom: 20px;
        right: 20px;
        background-color: var(--blue);
        color: #fff;
        padding: 10px 20px;
        border-radius: 5px;
        cursor: pointer;
        z-index: 9999;
    }

    .chat-interface {
        position: fixed;
        bottom: 80px;
        right: 20px;
        width: 300px;
        height: 400px;
        overflow: hidden;
        display: flex;
        flex-direction: column;
        background-color: var(--gray);
        border: 0;
        border-radius: 5px;
        z-index: 9998;
    }

    .card-header {
        background-color: var(--darkest-blue);
        color: var(--gray);
        padding: 10px 15px;
        font-size: 1.25rem;
    }

    .my-button {
        background-color: var(--darkest-blue);
        color: var(--gray);
    }

    .message {
        padding: 10px 15px;
        border-radius: 20px;
        margin-bottom: 10px;
        max-width: 80%;
        word-wrap: break-word;
    }

    .message.customer {
        background-color: var(--blue);
        align-self: flex-start;
    }

    .message.support {
        background-color: var(--dark-blue);
        color: white;
        align-self: flex-end;
    }

    .card-body.chat-body {
        flex: 1;
        overflow-y: auto;
        padding: 15px;
        background-color: #f8f9fa;
        display: flex;
        flex-direction: column;
    }
</style>
{#if !isUserAdmin()}
    <span class='chat-button' on:click={toggleChat}>
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat"
             viewBox="0 0 16 16">
      <path d="M2.678 11.894a1 1 0 0 1 .287.801 11 11 0 0 1-.398 2c1.395-.323 2.247-.697 2.634-.893a1 1 0 0 1 .71-.074A8 8 0 0 0 8 14c3.996 0 7-2.807 7-6s-3.004-6-7-6-7 2.808-7 6c0 1.468.617 2.83 1.678 3.894m-.493 3.905a22 22 0 0 1-.713.129c-.2.032-.352-.176-.273-.362a10 10 0 0 0 .244-.637l.003-.01c.248-.72.45-1.548.524-2.319C.743 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7-3.582 7-8 7a9 9 0 0 1-2.347-.306c-.52.263-1.639.742-3.468 1.105"/>
    </svg>
    </span>
{/if}
{#if showChat}
    <div class="card chat-interface">
        <div class="card-header d-flex align-items-center justify-content-between">
            <span>Customer support</span>
            <button type="button" class="btn my-button" aria-label="Close" on:click={toggleChat}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x"
                     viewBox="0 0 16 16">
                    <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
                </svg>
            </button>
        </div>
        <div class="card-body chat-body">
            {#each messages as msg}
                {#if msg.isAdminReply}
                    <div class="message support">
                        {msg.content}
                    </div>
                {:else}
                    <div class="message customer">
                        {msg.content}
                    </div>
                {/if}
            {/each}
        </div>
        <div class="card-footer d-flex">
            <input type="text" class="form-control" placeholder="Type your message..." bind:value={content}
                   on:keydown={handleKeyDown}>
            <button class="btn my-button" on:click={sendMessage}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-send"
                     viewBox="0 0 16 16">
                    <path d="M15.854.146a.5.5 0 0 1 .11.54l-5.819 14.547a.75.75 0 0 1-1.329.124l-3.178-4.995L.643 7.184a.75.75 0 0 1 .124-1.33L15.314.037a.5.5 0 0 1 .54.11ZM6.636 10.07l2.761 4.338L14.13 2.576zm6.787-8.201L1.591 6.602l4.339 2.76z"/>
                </svg>
            </button>
        </div>
    </div>
{/if}

